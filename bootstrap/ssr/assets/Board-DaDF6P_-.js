...
const _sfc_main = /* @__PURE__ */ Object.assign({ layout: null }, {
  ...
  setup(__props) {
    ...
    let clockTimer = null;
    const updateClock = () => {
      clock.value = (/* @__PURE__ */ new Date()).toLocaleTimeString("en-PK", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
    };
    let pollTimer = null;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: "kds",
        style: { "--kds-accent": ((_a = activeStation.value) == null ? void 0 : _a.color) ?? "#6366f1" }
      ...
      ssrRenderList(tickets.value, (ticket) => {
        ...
        ssrRenderList(ticket.items, (item) => {
          ...
          if (item.note) {
            _push(`<span class="kds__item-note" data-v-21b5432b>📝 ${ssrInterpolate(item.note)}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div></div><div class="kds__item-actions" data-v-21b5432b>`);
          if (item.kds_status === "sent") {
            _push(`<button class="kds__pill kds__pill--prep" data-v-21b5432b>Prep</button>`);
          } else {
            _push(`<!---->`);
          }
          if (item.kds_status === "preparing") {
            _push(`<button class="kds__pill kds__pill--ready" data-v-21b5432b>Ready</button>`);
          } else {
            _push(`<!---->`);
          }
          if (["sent", "preparing", "ready"].includes(item.kds_status)) {
            _push(`<button class="kds__pill kds__pill--bump" data-v-21b5432b>✓</button>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div></div>`);
      ...
      _push(`</div>`);
      if (!loading.value && tickets.value.length === 0) {
        _push(`<div class="kds__empty" data-v-21b5432b><div class="kds__empty-icon" data-v-21b5432b>✅</div><p data-v-21b5432b>No active tickets — kitchen is clear!</p></div>`);
      } else {
        _push(`<!---->`);
      }
      if (loading.value && tickets.value.length === 0) {
        _push(`<div class="kds__empty" data-v-21b5432b><div class="kds__spin" data-v-21b5432b></div><p data-v-21b5432b>Loading tickets…</p></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const Board = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-21b5432b"]]);
export {
  Board as default
};
