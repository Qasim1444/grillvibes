import { computed, ref, watch, onMounted, mergeProps, withCtx, unref, createVNode, toDisplayString, openBlock, createBlock, Fragment, createTextVNode, createCommentVNode, renderList, withDirectives, vModelSelect, vModelText, useSSRContext } from "vue";
...
const _sfc_main = /* @__PURE__ */ Object.assign({ layout: AdminLayout }, {
  ...
  setup(__props) {
    ...
    const viewFilter = ref(((_c = props.filters) == null ? void 0 : _c.view) ?? "");
    let timer = null;
    ...
    watch([search, categoryFilter, viewFilter], () => {
      clearTimeout(timer);
    ...
    const save = () => {
      const lines = form.lines.filter((l) => l.ingredient_id && Number(l.quantity) > 0).map((l) => ({ ingredient_id: l.ingredient_id, quantity: l.quantity, note: l.note || null }));
    ...
    const costOf = (id) => {
      var _a2;
      return Number(((_a2 = ingredientMap.value[id]) == null ? void 0 : _a2.average_cost) ?? 0);
    };
    ...
    const livePercent = computed(() => {
      var _a2;
      const price = Number(((_a2 = dish.value) == null ? void 0 : _a2.price) ?? 0);
      return price > 0 ? Math.round(liveCost.value / price * 1e4) / 100 : null;
    });
    const liveMargin = computed(() => {
      var _a2;
      return round2(Number(((_a2 = dish.value) == null ? void 0 : _a2.price) ?? 0) - liveCost.value);
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (missingDeepLink.value) {
        _push(`<p class="rc__notice" data-v-33641386> That dish isn&#39;t on this page — search for it by name to open its recipe. </p>`);
      } else {
        _push(`<!---->`);
      }
      ...
      _push(ssrRenderComponent(DataTable, {
        ...
        "cell:name": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<strong data-v-33641386${_scopeId}>${ssrInterpolate(row.name)}</strong>`);
            if (row.code) {
              _push2(`<span class="rc__muted rc__code" data-v-33641386${_scopeId}>${ssrInterpolate(row.code)}</span>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<span class="rc__cat" data-v-33641386${_scopeId}>${ssrInterpolate(row.category_name || "Uncategorised")}</span>`);
          } else {
            return [
        ...
        "cell:price": withCtx(({ value }, _push2, _parent2, _scopeId) => {
          if (_push2) {
        ...
        "cell:dish_cost": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (row.line_count === 0) {
              _push2(`<span class="ui-badge ui-badge--muted" data-v-33641386${_scopeId}>No recipe</span>`);
            } else {
              _push2(`<!--[-->${ssrInterpolate(fmt(row.dish_cost))} <span class="rc__muted rc__lines" data-v-33641386${_scopeId}>${ssrInterpolate(row.line_count)} ingredient(s)</span><!--]-->`);
            }
          } else {
            return [
              row.line_count === 0 ? (openBlock(), createBlock("span", {
                key: 0,
                class: "ui-badge ui-badge--muted"
              }, "No recipe")) : (openBlock(), createBlock(Fragment, { key: 1 }, [
                createTextVNode(toDisplayString(fmt(row.dish_cost)) + " ", 1),
                createVNode("span", { class: "rc__muted rc__lines" }, toDisplayString(row.line_count) + " ingredient(s)", 1)
              ], 64))
            ];
          }
        }),
        "cell:food_cost_percent": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (row.food_cost_percent === null) {
              _push2(`<span class="rc__muted" data-v-33641386${_scopeId}>—</span>`);
            } else {
              _push2(`<span class="${ssrRenderClass([pctBadge(row.food_cost_percent), "ui-badge"])}" data-v-33641386${_scopeId}>${ssrInterpolate(row.food_cost_percent)}%</span>`);
            }
          } else {
            return [
              row.food_cost_percent === null ? (openBlock(), createBlock("span", {
                key: 0,
                class: "rc__muted"
        ...
        "cell:margin": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (row.margin === null) {
              _push2(`<span class="rc__muted" data-v-33641386${_scopeId}>—</span>`);
            } else {
              _push2(`<span class="${ssrRenderClass(row.margin < 0 ? "rc__neg" : "")}" data-v-33641386${_scopeId}>${ssrInterpolate(fmt(row.margin))}</span>`);
            }
          } else {
            return [
              row.margin === null ? (openBlock(), createBlock("span", {
                key: 0,
                class: "rc__muted"
              }, "—")) : (openBlock(), createBlock("span", {
                key: 1,
        ...
        "cell:portions_available": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (row.portions_available === null) {
              _push2(`<span class="rc__muted" data-v-33641386${_scopeId}>—</span>`);
            } else {
              _push2(`<span class="${ssrRenderClass(row.portions_available === 0 ? "rc__neg" : row.portions_available < 10 ? "rc__low" : "")}" data-v-33641386${_scopeId}>${ssrInterpolate(row.portions_available)}</span>`);
            }
          } else {
            return [
              row.portions_available === null ? (openBlock(), createBlock("span", {
                key: 0,
                class: "rc__muted"
              }, "—")) : (openBlock(), createBlock("span", {
                key: 1,
        ...
        actions: withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-33641386${_scopeId}>${ssrInterpolate(unref(can)("inventory.recipes.update") ? row.line_count ? "Edit Recipe" : "Add Recipe" : "View Recipe")}</button>`);
          } else {
            return [
              createVNode("button", {
      ...
      _push(ssrRenderComponent(Modal, {
        ...
        footer: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<button class="ui-btn ui-btn--ghost" data-v-33641386${_scopeId}>${ssrInterpolate(editable.value ? "Cancel" : "Close")}</button>`);
            if (editable.value) {
              _push2(`<button class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(unref(form).processing || hasDuplicates.value) ? " disabled" : ""} data-v-33641386${_scopeId}> Save Recipe </button>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
            return [
              ...
              }, toDisplayString(editable.value ? "Cancel" : "Close"), 9, ["onClick"]),
          ...
          if (_push2) {
            if (dish.value) {
              ...
              ssrRenderList(unref(form).lines, (line, i) => {
                ...
                if (editable.value) {
                  _push2(`<button class="ui-btn ui-btn--danger ui-btn--sm" title="Remove line" data-v-33641386${_scopeId}>×</button>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</td></tr>`);
              ...
              _push2(`<!--]--></tbody>`);
              if (unref(form).lines.length) {
                _push2(`<tfoot data-v-33641386${_scopeId}><tr data-v-33641386${_scopeId}><td colspan="3" class="rc__right" data-v-33641386${_scopeId}><strong data-v-33641386${_scopeId}>Total</strong></td><td class="rc__right" data-v-33641386${_scopeId}><strong data-v-33641386${_scopeId}>${ssrInterpolate(fmt(liveCost.value))}</strong></td><td colspan="2" data-v-33641386${_scopeId}></td></tr></tfoot>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</table>`);
              if (unref(form).errors.lines) {
                _push2(`<p class="rc__err" data-v-33641386${_scopeId}>${ssrInterpolate(unref(form).errors.lines)}</p>`);
              } else {
                _push2(`<!---->`);
              }
              if (editable.value) {
                _push2(`<button class="ui-btn ui-btn--secondary ui-btn--sm rc__add" data-v-33641386${_scopeId}>+ Add Ingredient</button>`);
              } else {
                _push2(`<!---->`);
              }
              if (dish.value.portions_available !== null) {
                _push2(`<p class="rc__stockline" data-v-33641386${_scopeId}> Current stock can make <strong data-v-33641386${_scopeId}>${ssrInterpolate(dish.value.portions_available)}</strong> portion(s) of this dish as saved. </p>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div>`);
            ...
            return [
              dish.value ? (openBlock(), createBlock("div", {
                key: 0,
                class: "rc__build"
              }, [
                createVNode("div", { class: "rc__live" }, [
                  createVNode("div", { class: "rc__live-cell" }, [
                    createVNode("span", { class: "rc__live-label" }, "Selling Price"),
                    createVNode("span", { class: "rc__live-val" }, toDisplayString(fmt(dish.value.price)), 1)
                  ]),
                  createVNode("div", { class: "rc__live-cell" }, [
                    createVNode("span", { class: "rc__live-label" }, "Dish Cost"),
                    createVNode("span", { class: "rc__live-val" }, toDisplayString(fmt(liveCost.value)), 1)
                  ]),
                  createVNode("div", { class: "rc__live-cell" }, [
                    createVNode("span", { class: "rc__live-label" }, "Food Cost"),
                    createVNode("span", {
                      class: ["rc__live-val", livePctClass.value]
                    }, toDisplayString(livePercent.value === null ? "—" : livePercent.value + "%"), 3)
                  ]),
                  createVNode("div", { class: "rc__live-cell" }, [
                    createVNode("span", { class: "rc__live-label" }, "Margin"),
                    createVNode("span", {
                      class: ["rc__live-val", liveMargin.value < 0 ? "rc__neg" : "rc__pos"]
                    }, toDisplayString(fmt(liveMargin.value)), 3)
                  ])
                ]),
                ...
                createVNode("table", { class: "rc__table" }, [
                  ...
                  createVNode("tbody", null, [
                    ...
                    (openBlock(true), createBlock(Fragment, null, renderList(unref(form).lines, (line, i) => {
                      return openBlock(), createBlock("tr", { key: i }, [
                        ...
                        createVNode("td", { class: "rc__right" }, [
                          createVNode("strong", null, toDisplayString(fmt(lineCost(line))), 1)
                        ]),
                        createVNode("td", null, [
                          withDirectives(createVNode("input", {
                  ...
                  unref(form).lines.length ? (openBlock(), createBlock("tfoot", { key: 0 }, [
                    createVNode("tr", null, [
                      ...
                      createVNode("td", { class: "rc__right" }, [
                        createVNode("strong", null, toDisplayString(fmt(liveCost.value)), 1)
                      ]),
...
const Recipes = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-33641386"]]);
export {
  Recipes as default
};
