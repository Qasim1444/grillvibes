import { computed, ref, watch, mergeProps, withCtx, unref, createTextVNode, createVNode, openBlock, createBlock, createCommentVNode, toDisplayString, Fragment, renderList, withDirectives, vModelSelect, vModelText, useSSRContext } from "vue";
...
const _sfc_main = /* @__PURE__ */ Object.assign({ layout: AdminLayout }, {
  ...
  setup(__props) {
    ...
    const vendorFilter = ref(((_c = props.filters) == null ? void 0 : _c.vendor_id) ?? "");
    let timer = null;
    ...
    const unitOf = (id) => {
      var _a2;
      return ((_a2 = ingredientMap.value[id]) == null ? void 0 : _a2.unit) ?? "";
    };
    ...
    const selectedVendorTerms = computed(() => {
      var _a2;
      return ((_a2 = props.vendors.find((v) => String(v.id) === String(form.vendor_id))) == null ? void 0 : _a2.payment_terms) ?? "";
    });
    ...
    const fmtDate = (v) => v ? (/* @__PURE__ */ new Date(v + "T00:00:00")).toLocaleDateString("en-PK", { day: "2-digit", month: "short", year: "numeric" }) : "—";
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_sfc_main$1, {
        ...
        actions: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (unref(can)("procurement.purchase-orders.create")) {
              _push2(`<button class="ui-btn ui-btn--primary" data-v-25221502${_scopeId}> + New Purchase Order </button>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
      ...
      _push(ssrRenderComponent(DataTable, {
        ...
        "cell:po_number": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<strong data-v-25221502${_scopeId}>${ssrInterpolate(row.po_number)}</strong><span class="po__sub" data-v-25221502${_scopeId}>${ssrInterpolate(row.line_count)} line(s)</span>`);
          } else {
            return [
              createVNode("strong", null, toDisplayString(row.po_number), 1),
              createVNode("span", { class: "po__sub" }, toDisplayString(row.line_count) + " line(s)", 1)
            ];
          }
        }),
        "cell:vendor_name": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`${ssrInterpolate(row.vendor_name || "—")} `);
            if (row.created_by_name) {
              _push2(`<span class="po__sub" data-v-25221502${_scopeId}>by ${ssrInterpolate(row.created_by_name)}</span>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
            return [
              createTextVNode(toDisplayString(row.vendor_name || "—") + " ", 1),
        ...
        "cell:status": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<span class="${ssrRenderClass([statusBadge(row.status), "ui-badge"])}" data-v-25221502${_scopeId}>${ssrInterpolate(row.status_label)}</span>`);
            if (row.is_overdue) {
              _push2(`<span class="ui-badge ui-badge--danger po__pill" data-v-25221502${_scopeId}>Overdue</span>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
            return [
              createVNode("span", {
                class: ["ui-badge", statusBadge(row.status)]
              }, toDisplayString(row.status_label), 3),
              row.is_overdue ? (openBlock(), createBlock("span", {
                key: 0,
        ...
        "cell:expected_at": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<span class="${ssrRenderClass(row.is_overdue ? "po__neg" : "")}" data-v-25221502${_scopeId}>${ssrInterpolate(fmtDate(row.expected_at))}</span>`);
            if (row.ordered_at) {
              _push2(`<span class="po__sub" data-v-25221502${_scopeId}>sent ${ssrInterpolate(fmtDate(row.ordered_at))}</span>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
            return [
              createVNode("span", {
                class: row.is_overdue ? "po__neg" : ""
              }, toDisplayString(fmtDate(row.expected_at)), 3),
              row.ordered_at ? (openBlock(), createBlock("span", {
                key: 0,
                class: "po__sub"
        ...
        "cell:total": withCtx(({ value }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<strong data-v-25221502${_scopeId}>${ssrInterpolate(fmt(value))}</strong>`);
          } else {
            return [
        ...
        "cell:outstanding_value": withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (row.outstanding_value > 0) {
              _push2(`<span class="po__warn" data-v-25221502${_scopeId}>${ssrInterpolate(fmt(row.outstanding_value))}</span>`);
            } else {
              _push2(`<span class="po__muted" data-v-25221502${_scopeId}>—</span>`);
            }
          } else {
            return [
              row.outstanding_value > 0 ? (openBlock(), createBlock("span", {
                key: 0,
                class: "po__warn"
        ...
        actions: withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-25221502${_scopeId}>View</button>`);
            if (row.is_draft && unref(can)("procurement.purchase-orders.update")) {
              _push2(`<button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-25221502${_scopeId}>Edit</button>`);
            } else {
              _push2(`<!---->`);
            }
            if (row.is_draft && unref(can)("procurement.purchase-orders.approve")) {
              _push2(`<button class="ui-btn ui-btn--primary ui-btn--sm" data-v-25221502${_scopeId}>Send</button>`);
            } else {
              _push2(`<!---->`);
            }
            if (row.is_receivable && unref(can)("procurement.goods-receipts.create")) {
              _push2(ssrRenderComponent(unref(Link), {
            ...
            if (!row.is_draft && row.is_receivable && unref(can)("procurement.purchase-orders.update")) {
              _push2(`<button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-25221502${_scopeId}>Cancel</button>`);
            } else {
              _push2(`<!---->`);
            }
            if (row.is_draft && unref(can)("procurement.purchase-orders.delete")) {
              _push2(`<button class="ui-btn ui-btn--danger ui-btn--sm" data-v-25221502${_scopeId}>Delete</button>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
      ...
      _push(ssrRenderComponent(Modal, {
        ...
        footer: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<button class="ui-btn ui-btn--ghost" data-v-25221502${_scopeId}>Cancel</button><button class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(unref(form).processing || hasDuplicates.value || !validLines.value.length) ? " disabled" : ""} data-v-25221502${_scopeId}>${ssrInterpolate(unref(form).id ? "Save Changes" : "Save Draft")}</button>`);
          } else {
            return [
              createVNode("button", {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="form-grid-2" data-v-25221502${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$2, {
              ...
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`<option value="" data-v-25221502${_scopeId2}>Select a vendor…</option><!--[-->`);
                  ssrRenderList(props.vendors, (v) => {
                    _push3(`<option${ssrRenderAttr("value", v.id)} data-v-25221502${_scopeId2}>${ssrInterpolate(v.name)}</option>`);
                  });
                  _push3(`<!--]-->`);
                } else {
                  return [
                    createVNode("option", { value: "" }, "Select a vendor…"),
            ...
            _push2(`</div>`);
            if (selectedVendorTerms.value) {
              _push2(`<p class="po__terms" data-v-25221502${_scopeId}>Payment terms: ${ssrInterpolate(selectedVendorTerms.value)}</p>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<table class="po__table" data-v-25221502${_scopeId}><thead data-v-25221502${_scopeId}><tr data-v-25221502${_scopeId}><th style="${ssrRenderStyle({ "width": "36%" })}" data-v-25221502${_scopeId}>Ingredient</th><th style="${ssrRenderStyle({ "width": "18%" })}" data-v-25221502${_scopeId}>Quantity</th><th style="${ssrRenderStyle({ "width": "18%" })}" data-v-25221502${_scopeId}>Unit Cost</th><th class="po__right" data-v-25221502${_scopeId}>Line Total</th><th style="${ssrRenderStyle({ "width": "40px" })}" data-v-25221502${_scopeId}></th></tr></thead><tbody data-v-25221502${_scopeId}>`);
            if (!unref(form).lines.length) {
              _push2(`<tr data-v-25221502${_scopeId}><td colspan="5" class="po__muted po__empty" data-v-25221502${_scopeId}>Add the first line to start this order.</td></tr>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<!--[-->`);
            ssrRenderList(unref(form).lines, (line, i) => {
              _push2(`<tr data-v-25221502${_scopeId}><td data-v-25221502${_scopeId}><select class="ui-input" data-v-25221502${_scopeId}><option value="" data-v-25221502${ssrIncludeBooleanAttr(Array.isArray(line.ingredient_id) ? ssrLooseContain(line.ingredient_id, "") : ssrLooseEqual(line.ingredient_id, "")) ? " selected" : ""}${_scopeId}>Select…</option><!--[-->`);
              ssrRenderList(props.ingredients, (ing) => {
                _push2(`<option${ssrRenderAttr("value", ing.id)} data-v-25221502${ssrIncludeBooleanAttr(Array.isArray(line.ingredient_id) ? ssrLooseContain(line.ingredient_id, ing.id) : ssrLooseEqual(line.ingredient_id, ing.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(ing.name)}</option>`);
              });
              _push2(`<!--]--></select>`);
              if (isDuplicate(line, i)) {
                _push2(`<span class="po__err" data-v-25221502${_scopeId}>Already listed above</span>`);
              } else {
                _push2(`<!---->`);
              }
            ...
            _push2(`<!--]--></tbody>`);
            if (unref(form).lines.length) {
              _push2(`<tfoot data-v-25221502${_scopeId}><tr data-v-25221502${_scopeId}><td colspan="3" class="po__right" data-v-25221502${_scopeId}><strong data-v-25221502${_scopeId}>Order Total</strong></td><td class="po__right" data-v-25221502${_scopeId}><strong data-v-25221502${_scopeId}>${ssrInterpolate(fmt(orderTotal.value))}</strong></td><td data-v-25221502${_scopeId}></td></tr></tfoot>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</table>`);
            if (unref(form).errors.lines) {
              _push2(`<p class="po__err" data-v-25221502${_scopeId}>${ssrInterpolate(unref(form).errors.lines)}</p>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<button class="ui-btn ui-btn--secondary ui-btn--sm po__add" data-v-25221502${_scopeId}>+ Add Line</button>`);
            _push2(ssrRenderComponent(_sfc_main$2, {
            ...
            _push2(`<p class="po__hint" data-v-25221502${_scopeId}> Saved as a draft — it commits nothing until you send it. Unit costs here are what you expect to pay; the price that actually values your stock is the one on the delivery invoice. </p>`);
          } else {
            return [
              ...
              createVNode("table", { class: "po__table" }, [
                ...
                createVNode("tbody", null, [
                  !unref(form).lines.length ? (openBlock(), createBlock("tr", { key: 0 }, [
                    createVNode("td", {
                      colspan: "5",
                      class: "po__muted po__empty"
                    }, "Add the first line to start this order.")
                    ...
                    return openBlock(), createBlock("tr", { key: i }, [
                      createVNode("td", null, [
                        withDirectives(createVNode("select", {
                          ...
                          (openBlock(true), createBlock(Fragment, null, renderList(props.ingredients, (ing) => {
                            return openBlock(), createBlock("option", {
                              key: ing.id,
                              value: ing.id
                          ...
                          [vModelSelect, line.ingredient_id]
                        ]),
                        isDuplicate(line, i) ? (openBlock(), createBlock("span", {
                          key: 0,
                          class: "po__err"
                        }, "Already listed above")) : createCommentVNode("", true)
                      ]),
                      createVNode("td", null, [
                        createVNode("div", { class: "po__qty" }, [
                      ...
                      createVNode("td", { class: "po__right" }, [
                        createVNode("strong", null, toDisplayString(fmt(lineTotal(line))), 1)
                      ]),
                      createVNode("td", null, [
                        createVNode("button", {
                          class: "ui-btn ui-btn--danger ui-btn--sm",
                ...
                unref(form).lines.length ? (openBlock(), createBlock("tfoot", { key: 0 }, [
                  createVNode("tr", null, [
                    createVNode("td", {
                      colspan: "3",
                      class: "po__right"
                    ...
                    createVNode("td", { class: "po__right" }, [
                      createVNode("strong", null, toDisplayString(fmt(orderTotal.value)), 1)
                    ]),
                    createVNode("td")
                  ])
                ])) : createCommentVNode("", true)
      ...
      _push(ssrRenderComponent(Modal, {
        ...
        title: ((_a2 = viewing.value) == null ? void 0 : _a2.po_number) ?? "Purchase Order",
        width: "900px"
      }, {
        footer: withCtx((_, _push2, _parent2, _scopeId) => {
          var _a3, _b2;
          if (_push2) {
            _push2(`<button class="ui-btn ui-btn--ghost" data-v-25221502${_scopeId}>Close</button>`);
            if (((_a3 = viewing.value) == null ? void 0 : _a3.is_receivable) && unref(can)("procurement.goods-receipts.create")) {
              _push2(ssrRenderComponent(unref(Link), {
                ...
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`Receive Delivery`);
                  } else {
                    return [
                      createTextVNode("Receive Delivery")
                    ];
                  }
                }),
                _: 1
            ...
            return [
              ...
              ((_b2 = viewing.value) == null ? void 0 : _b2.is_receivable) && unref(can)("procurement.goods-receipts.create") ? (openBlock(), createBlock(unref(Link), {
                key: 0,
                href: `/procurement/goods-receipts?po=${viewing.value.id}`,
                class: "ui-btn ui-btn--primary"
              }, {
              ...
              viewing.value ? (openBlock(), createBlock("div", { key: 0 }, [
                createVNode("div", { class: "po__meta" }, [
                  createVNode("div", null, [
                    createVNode("span", { class: "po__meta-label" }, "Vendor"),
                    createVNode("span", null, toDisplayString(viewing.value.vendor_name || "—"), 1)
                  ]),
                  createVNode("div", null, [
                    createVNode("span", { class: "po__meta-label" }, "Status"),
                    createVNode("span", {
                      class: ["ui-badge", statusBadge(viewing.value.status)]
                    }, toDisplayString(viewing.value.status_label), 3)
                  ]),
                  createVNode("div", null, [
                    createVNode("span", { class: "po__meta-label" }, "Sent"),
                    createVNode("span", null, toDisplayString(fmtDate(viewing.value.ordered_at)), 1)
                  ]),
                  createVNode("div", null, [
                    createVNode("span", { class: "po__meta-label" }, "Expected"),
                    createVNode("span", {
                      class: viewing.value.is_overdue ? "po__neg" : ""
                    }, toDisplayString(fmtDate(viewing.value.expected_at)), 3)
                  ]),
                  createVNode("div", null, [
                    createVNode("span", { class: "po__meta-label" }, "Total"),
                    createVNode("span", null, [
                      createVNode("strong", null, toDisplayString(fmt(viewing.value.total)), 1)
                    ])
                  ]),
                  createVNode("div", null, [
                    createVNode("span", { class: "po__meta-label" }, "Outstanding"),
                    createVNode("span", {
                      class: viewing.value.outstanding_value > 0 ? "po__warn" : ""
                    }, toDisplayString(fmt(viewing.value.outstanding_value)), 3)
                  ])
                ]),
                createVNode("table", { class: "po__table" }, [
                  createVNode("thead", null, [
                  ...
                  createVNode("tbody", null, [
                    (openBlock(true), createBlock(Fragment, null, renderList(viewing.value.lines, (line) => {
                      return openBlock(), createBlock("tr", {
                        ...
                        createVNode("td", {
                          class: ["po__right", line.outstanding_qty > 0 ? "po__warn" : "po__muted"]
                        }, toDisplayString(qty(line.outstanding_qty)), 3),
                        createVNode("td", { class: "po__right po__muted" }, toDisplayString(fmt(line.unit_cost)), 1),
                        createVNode("td", { class: "po__right" }, [
                          createVNode("strong", null, toDisplayString(fmt(line.line_total)), 1)
                        ])
                      ]);
                    }), 128))
                  ])
                ...
                viewing.value.notes ? (openBlock(), createBlock("p", {
                  ...
                  createVNode("span", { class: "po__meta-label" }, "Notes"),
                  createTextVNode(toDisplayString(viewing.value.notes), 1)
                ])) : createCommentVNode("", true)
              ])) : createCommentVNode("", true)
            ];
          }
      ...
      _push(ssrRenderComponent(Modal, {
        ...
        footer: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<button class="ui-btn ui-btn--ghost" data-v-25221502${_scopeId}>Keep Open</button><button class="ui-btn ui-btn--danger"${ssrIncludeBooleanAttr(unref(cancelForm).processing) ? " disabled" : ""} data-v-25221502${_scopeId}>Cancel Order</button>`);
          } else {
            return [
              createVNode("button", {
                class: "ui-btn ui-btn--ghost",
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          var _a3, _b2;
          if (_push2) {
            _push2(`<p class="po__hint" data-v-25221502${_scopeId}> Cancelling abandons whatever is still outstanding on ${ssrInterpolate((_a3 = cancelling.value) == null ? void 0 : _a3.po_number)}. Anything already delivered keeps its stock and its cost — this only closes the order. </p>`);
            _push2(ssrRenderComponent(_sfc_main$2, {
            ...
            return [
              createVNode("p", { class: "po__hint" }, " Cancelling abandons whatever is still outstanding on " + toDisplayString((_b2 = cancelling.value) == null ? void 0 : _b2.po_number) + ". Anything already delivered keeps its stock and its cost — this only closes the order. ", 1),
              createVNode(_sfc_main$2, {
                modelValue: unref(cancelForm).reason,
                "onUpdate:modelValue": ($event) => unref(cancelForm).reason = $event,
                label: "Reason",
...
const PurchaseOrders = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-25221502"]]);
export {
