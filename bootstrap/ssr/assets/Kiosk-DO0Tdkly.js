...
import { ssrRenderAttrs, ssrInterpolate, ssrRenderStyle, ssrRenderList, ssrRenderAttr, ssrRenderClass, ssrIncludeBooleanAttr } from "vue/server-renderer";
  ...
  setup(__props) {
    ...
    onMounted(async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(((_a = await res.json()) == null ? void 0 : _a.error) ?? "Kiosk unavailable.");
    ...
    const cartQty = (id) => {
      var _a;
      return ((_a = cart.value.find((l) => l.fooditems_id === id)) == null ? void 0 : _a.quantity) ?? 0;
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (state.value === "loading") {
        _push(`<div class="kiosk__splash kiosk__splash--loading" data-v-be986fb4><div class="kiosk__spin" data-v-be986fb4></div></div>`);
      } else if (state.value === "error") {
        _push(`<div class="kiosk__splash" data-v-be986fb4><p class="kiosk__error" data-v-be986fb4>${ssrInterpolate(errorMsg.value)}</p></div>`);
      } else if (state.value === "attract") {
        _push(`<div class="kiosk__attract" data-v-be986fb4>`);
        if (config.value.splash_image) {
          _push(`<div class="kiosk__splash-bg" style="${ssrRenderStyle({ backgroundImage: `url(${config.value.splash_image})` })}" data-v-be986fb4></div>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (config.value.require_name) {
          _push(`<input${ssrRenderAttr("value", guestName.value)} class="kiosk__big-input" placeholder="Your name" data-v-be986fb4>`);
        } else {
          _push(`<!---->`);
        }
        if (config.value.require_phone) {
          _push(`<input${ssrRenderAttr("value", guestPhone.value)} class="kiosk__big-input" placeholder="Phone number" data-v-be986fb4>`);
        } else {
          _push(`<!---->`);
        }
        ...
        ssrRenderList(filteredItems.value, (item) => {
          ...
          if (item.image) {
            _push(`<img${ssrRenderAttr("src", item.image)}${ssrRenderAttr("alt", item.name)} data-v-be986fb4>`);
          } else {
            _push(`<span data-v-be986fb4>🍽</span>`);
          }
          ...
          if (cartQty(item.id)) {
            _push(`<span class="kiosk__item-badge" data-v-be986fb4>${ssrInterpolate(cartQty(item.id))}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</button>`);
        ...
        if (!cart.value.length) {
          _push(`<div class="kiosk__cart-empty" data-v-be986fb4>Tap items to add them</div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--[-->`);
        ...
        if (cart.value.length) {
          _push(`<button class="kiosk__checkout-btn"${ssrIncludeBooleanAttr(placing.value) ? " disabled" : ""} data-v-be986fb4>${ssrInterpolate(placing.value ? "Processing…" : "Place Order →")}</button>`);
        } else {
          _push(`<!---->`);
        }
        if (orderError.value) {
          _push(`<p class="kiosk__order-error" data-v-be986fb4>${ssrInterpolate(orderError.value)}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div></div>`);
      } else if (state.value === "confirmed") {
        _push(`<div class="kiosk__confirmed" data-v-be986fb4><div class="kiosk__confirmed-icon" data-v-be986fb4>✅</div><h2 data-v-be986fb4>Order Confirmed!</h2><p class="kiosk__confirmed-num" data-v-be986fb4>${ssrInterpolate(confirmOrder.value.order_number)}</p><p class="kiosk__confirmed-total" data-v-be986fb4>${ssrInterpolate(fmt(confirmOrder.value.grand_total))}</p><p class="kiosk__confirmed-msg" data-v-be986fb4>${ssrInterpolate(confirmOrder.value.message)}</p><div class="kiosk__countdown" data-v-be986fb4> Returning to start in <strong data-v-be986fb4>${ssrInterpolate(countdown.value)}</strong>s </div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const Kiosk = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-be986fb4"]]);
export {
  Kiosk as default
};
