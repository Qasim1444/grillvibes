import { reactive, computed, unref, withCtx, createVNode, toDisplayString, useSSRContext, ref, onMounted, onBeforeUnmount, mergeProps, createTextVNode } from "vue";
import { ssrRenderClass, ssrRenderList, ssrInterpolate, ssrRenderComponent, ssrRenderAttrs, ssrRenderSlot } from "vue/server-renderer";
...
function usePermissions() {
  ...
  const permissions = computed(() => {
    var _a;
    return ((_a = page.props.auth) == null ? void 0 : _a.permissions) ?? [];
  });
  const roles = computed(() => {
    var _a;
    return ((_a = page.props.auth) == null ? void 0 : _a.roles) ?? [];
  });
...
const _sfc_main$2 = {
  ...
  setup(__props) {
    const page = usePage();
    const { state: state2, closeMobile } = useSidebar();
    const { can } = usePermissions();
    const currentPath = computed(() => page.url.split("?")[0]);
    const isActive = (to) => currentPath.value === to || currentPath.value.startsWith(to + "/");
    const icon = {
    ...
    const visibleGroups = computed(
      () => groups.map((group) => ({ ...group, items: group.items.filter((item) => can(item.permission)) })).filter((group) => group.items.length > 0)
    );
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[--><aside class="${ssrRenderClass([{ "sidebar--collapsed": unref(state2).collapsed, "sidebar--mobile-open": unref(state2).mobileOpen }, "sidebar"])}" data-v-2a0a8f36><div class="sidebar__brand" data-v-2a0a8f36><div class="sidebar__logo" data-v-2a0a8f36>J</div><span class="sidebar__brand-name" data-v-2a0a8f36>KitchenOS Admin</span></div><nav class="sidebar__nav" data-v-2a0a8f36><!--[-->`);
      ssrRenderList(visibleGroups.value, (group) => {
        _push(`<!--[--><div class="sidebar__group-title" data-v-2a0a8f36>${ssrInterpolate(group.title)}</div><!--[-->`);
        ssrRenderList(group.items, (item) => {
          _push(ssrRenderComponent(unref(Link), {
            key: item.to,
            href: item.to,
            class: ["sidebar__link", { "sidebar__link--active": isActive(item.to) }],
            onClick: unref(closeMobile)
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<span class="sidebar__icon" data-v-2a0a8f36${_scopeId}>${item.icon ?? ""}</span><span class="sidebar__label" data-v-2a0a8f36${_scopeId}>${ssrInterpolate(item.label)}</span>`);
              } else {
                return [
                  createVNode("span", {
                    class: "sidebar__icon",
                  ...
                  createVNode("span", { class: "sidebar__label" }, toDisplayString(item.label), 1)
                ];
              }
            }),
            _: 2
      ...
      _push(`<!--]--></nav></aside>`);
      if (unref(state2).mobileOpen) {
        _push(`<div class="sidebar__backdrop" data-v-2a0a8f36></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<!--]-->`);
...
const Sidebar = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-2a0a8f36"]]);
const _sfc_main$1 = {
  __name: "Topbar",
  __ssrInlineRender: true,
  setup(__props) {
    ...
    const user = computed(() => {
      var _a;
      return ((_a = page.props.auth) == null ? void 0 : _a.user) ?? { name: "User" };
    });
    ...
    const currentBranch = computed(() => {
      var _a;
      return ((_a = page.props.auth) == null ? void 0 : _a.branch) ?? null;
    });
    const currentBranchId = computed(() => {
      var _a;
      return ((_a = currentBranch.value) == null ? void 0 : _a.id) ?? null;
    });
    const currentBranchName = computed(() => {
      var _a;
      return ((_a = currentBranch.value) == null ? void 0 : _a.name) ?? "Select Outlet";
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<header${ssrRenderAttrs(mergeProps({ class: "topbar" }, _attrs))} data-v-7953cf89><div class="topbar__left" data-v-7953cf89><button class="topbar__toggle" type="button" aria-label="Toggle menu" data-v-7953cf89><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" data-v-7953cf89><path d="M3 6h18M3 12h18M3 18h18" data-v-7953cf89></path></svg></button><h1 class="topbar__title" data-v-7953cf89>${ssrInterpolate(pageTitle.value)}</h1></div><div class="topbar__right" data-v-7953cf89>`);
      if (branches.value.length) {
        _push(`<div class="topbar__branch" data-v-7953cf89><button class="topbar__branch-btn" type="button" data-v-7953cf89><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-v-7953cf89><path d="M3 21h18M6 21V8l6-4 6 4v13M10 21v-5h4v5" data-v-7953cf89></path></svg><span class="topbar__branch-name" data-v-7953cf89>${ssrInterpolate(currentBranchName.value)}</span><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" data-v-7953cf89><path d="m6 9 6 6 6-6" data-v-7953cf89></path></svg></button>`);
        if (branchMenuOpen.value) {
          _push(`<div class="topbar__menu topbar__menu--branch" data-v-7953cf89><!--[-->`);
          ssrRenderList(branches.value, (b) => {
            _push(`<button class="${ssrRenderClass([{ "topbar__branch-item--active": b.id === currentBranchId.value }, "topbar__menu-item topbar__branch-item"])}" type="button" data-v-7953cf89><span data-v-7953cf89>${ssrInterpolate(b.name)}</span>`);
            if (b.id === currentBranchId.value) {
              _push(`<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" data-v-7953cf89><path d="M20 6 9 17l-5-5" data-v-7953cf89></path></svg>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</button>`);
      ...
      _push(`<div class="topbar__user" data-v-7953cf89><button class="topbar__user-btn" type="button" data-v-7953cf89><span class="topbar__avatar" data-v-7953cf89>${ssrInterpolate(initials.value)}</span><span class="topbar__user-name" data-v-7953cf89>${ssrInterpolate(user.value.name)}</span><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" data-v-7953cf89><path d="m6 9 6 6 6-6" data-v-7953cf89></path></svg></button>`);
      if (menuOpen.value) {
        _push(`<div class="topbar__menu" data-v-7953cf89>`);
        _push(ssrRenderComponent(unref(Link), {
          href: "/profile",
        ...
        _push(`<div class="topbar__menu-divider" data-v-7953cf89></div><button class="topbar__menu-item topbar__menu-item--danger" data-v-7953cf89>Logout</button></div>`);
      } else {
        _push(`<!---->`);
      }
...
const Topbar = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-7953cf89"]]);
const _sfc_main = {
  __name: "AdminLayout",
  __ssrInlineRender: true,
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: ["admin", { "admin--collapsed": unref(state2).collapsed }]
      }, _attrs))} data-v-26586321>`);
...
const AdminLayout = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-26586321"]]);
export {
  AdminLayout as A,
  usePermissions as u
};
