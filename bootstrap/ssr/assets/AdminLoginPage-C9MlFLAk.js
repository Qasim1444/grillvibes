...
var _sfc_main = {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (authError.value) _push(`<div class="error-banner" role="alert" data-v-0769ab5e>${ssrInterpolate(authError.value)}</div>`);
			else _push(`<!---->`);
			...
			if (errors.email) _push(`<small class="field-error" data-v-0769ab5e>${ssrInterpolate(errors.email)}</small>`);
			else _push(`<!---->`);
			...
			if (errors.password) _push(`<small class="field-error" data-v-0769ab5e>${ssrInterpolate(errors.password)}</small>`);
			else _push(`<!---->`);
...
var AdminLoginPage_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["__scopeId", "data-v-0769ab5e"]]);
//#endregion
