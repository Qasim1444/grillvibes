	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			ssrRenderList(plans, (plan) => {
				...
				if (plan.featured) _push(`<div class="pricing-card__label" data-v-9caabf4c>Most popular</div>`);
				else _push(`<!---->`);
			...
			ssrRenderList(pricingFaqs, (item, index) => {
				...
				if (expandedFaq.value === index) _push(`<div class="faq-body" data-v-9caabf4c>${ssrInterpolate(item.a)}</div>`);
				else _push(`<!---->`);
				_push(`</div>`);
			});
...
var PricingPage_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["__scopeId", "data-v-9caabf4c"]]);
//#endregion
